...
const AdhkarScreen = () => {
  ...
  return (
    <div className={`flex flex-col h-full bg-transparent p-6 relative z-10 ${lang === 'ar' ? 'rtl' : ''}`}>
      <header className="text-center mb-8 pt-4">
        <h1 className="text-[28px] font-arabic font-bold text-moroccan-red drop-shadow-sm">
          سبق المفردون
        </h1>
      </header>

      {/* Category selector */}
      <div className="flex space-x-2 space-x-reverse overflow-x-auto pb-6 no-scrollbar">
        {CATEGORIES.map(cat => (
          <button
            key={cat.id}
            onClick={() => handleCategoryChange(cat.id)}
            className={`whitespace-nowrap chip flex items-center space-x-2 space-x-reverse ${
        ...
        <AnimatePresence mode="wait">
          {currentDhikr && !isCompletedModalOpen ? (
            <motion.div
              ...
              className="w-full h-full max-w-md spiritual-card relative flex flex-col p-8 tap-effect cursor-pointer select-none"
              onClick={handleTap}
            >
              <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              className="w-full h-full max-w-md spiritual-card flex flex-col items-center justify-center p-8 text-center space-y-6"
            >
  ...
  return (
    <div className={`flex flex-col h-full bg-transparent p-6 pb-24 relative z-10 ${lang === 'ar' ? 'rtl' : ''}`}>
      <header className="pt-4 mb-8">
        <h2 className="text-3xl font-display font-bold text-moroccan-green">{t('wisdom')}</h2>
...
const ProfileScreen = () => {
  ...
  return (
    <div className={`flex flex-col h-full bg-transparent p-8 pb-32 overflow-y-auto relative z-10 ${lang === 'ar' ? 'rtl' : ''}`}>
      <header className="flex flex-col items-center pt-8 mb-12">
        <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-moroccan-red to-moroccan-gold p-1 shadow-xl">
